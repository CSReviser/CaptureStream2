...
**近年のmacOSではセキュリティ対策が強化されており、Appleによる公証を受けていないアプリ（本ソフトを含む）を実行する際に警告が表示されたり、起動が制限されたりする場合があります。**
**導入にあたっては、macOSのセキュリティ仕様をご理解のうえ、ご自身の判断でご利用ください。
警告の回避方法や導入手順については、以下のリンクをご参照ください。**
...
**プログラムのソースコードはGitHubで公開しております。ご利用の環境で実行ファイルをビルドすることが可能ですが、セキュリティ警告を完全に回避できることを保証するものではありません。なお、GitHubの macos-14 環境でビルド可能であることは確認済みです。**      
...
#### 　　 
**Macユーザーからの報告例（参考）**                                   
  - **macOS Tahoe 26.3.1 M機**
  - **macOS Sequoia 15.3.2 M機**      
  - **macOS Sequoia 15.7.4 Intel機**
  - **macOS Sequoia 15.0 M3機**            
  - **macOS Sonoma 14.4.1 Intel機**
  - **macOS Sonoma 14.8.4 M1機**
