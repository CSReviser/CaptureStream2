...
* **ffmpegが存在するフォルダのパスを指定可能にしました。指定しない場合の初期値は従来と同じとなります。環境によって生じるffmpeg関連の不具合対策です。環境にあったffmpegを準備してください。設定方法は[ffmpegパス設定](./SETTING_ffmpeg)**
