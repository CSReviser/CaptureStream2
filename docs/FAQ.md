...
* **A:　Qt5版で使用可能ですが、Windows8.1以前はMicrosoftのサポートが終了しているので使用はおすすめできません。また語学講座CS2の必要要件は2025年度現在Qt6.9以降なので、通常の実行ファイルはWindows8.1以前には非対応です。[QtバージョンとWindows対応](./Qt_vs_OS#windows)を参照ください。Qt5版は配布しているzipのファイル名に「qt5」という文字が含まれます。自己責任でご使用ください。「ffmpeg.exe」は使用環境で動作するものを準備してください。**
